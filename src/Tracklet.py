...
class Tracklet:
	...
	def drawseqbb(self,dim='x',savename=None):
		self._seqdraw(self.seqbb,self.bbr,dim,savename)

	def drawrfseqbb(self,dim='x',savename=None):
		if self.seqbbsim is None:
			print('Refine first')
			return
		self._seqdraw(self.seqbbsim,self.bbs,dim,savename)

	def _seqdraw(self,seqbb,bb,dim,savename=None):
		for seqi in range(len(seqbb)):
			...
			pyplot.plot(xseq,sliceq, 'o', label='Trace%d'%seqi)
		#pyplot.legend(loc="lower right")
		#pyplot.xlim([200,550])
		pyplot.ylabel('slice number of each bounding box')
		pyplot.xlabel(dim+' position of each bounding box')
	...
	def refbb(self,config=None):
		...
		for seqi in range(len(seqbb)):
			...
			availseqj = [i for i in range(len(startseqbb)) if startseqbb[i]>=endseqbb[seqi] and i not in connectstart+[seqi]]
			for seqj in availseqj:
				...
				closs = (1-ciou)*weightiou + cbbfit*weightbbfit + cgap*weightgap
				if DEBUG:
					print(seqi,'(%d,%d-%d,%d)'%(seqbb[seqi][-1][0]-seqbb[seqi][0][0],seqbb[seqi][0][0],seqbb[seqi][-1][0],self.bbr[seqbb[seqi][0][0]][seqbb[seqi][0][1]].x),
						 seqj,'(%d,%d-%d,%d)'%(seqbb[seqj][-1][0]-seqbb[seqj][0][0],seqbb[seqj][0][0],seqbb[seqj][-1][0],self.bbr[seqbb[seqj][0][0]][seqbb[seqj][0][1]].x),
			...
			if seqminind==-1:
				...
				ciminloss = np.inf
				seqiminind = -1
				availseqii = [i for i in range(len(endseqbb)) if endseqbb[i]<=startseqbb[seqminind] and i not in connectend+[seqminind]]
		...
		if self.RESTRICT_BRANCH_NUM==0:
			...
			xpos0 = [bbint[seqbbint[seqmax[0]][seqslicei][0]][seqbbint[seqmax[0]][seqslicei][1]].x for seqslicei in range(len(seqbbint[seqmax[0]]))]
			meanx0 = np.mean(xpos0)
			stdx0 = np.std(xpos0)
			xpos1 = [bbint[seqbbint[seqmax[1]][seqslicei][0]][seqbbint[seqmax[1]][seqslicei][1]].x for seqslicei in range(len(seqbbint[seqmax[1]]))]
			meanx1 = np.mean(xpos1)
			stdx1 = np.std(xpos1)
			print('Top tracklets','len',len(seqbbint[seqmax[0]]),'xmean',meanx0,'+-',stdx0,'len',len(seqbbint[seqmax[1]]),'xmean',meanx1,'+-',stdx1)
			for seqi in range(len(seqbbint)):
				if len(seqbbint[seqi])<3:
					if DEBUG:
						print('skip seqi',seqi,'(%d,%d-%d,%d)'%(seqbbint[seqi][-1][0]-seqbbint[seqi][0][0],seqbbint[seqi][0][0],seqbbint[seqi][-1][0],self.bbr[seqbbint[seqi][0][0]][seqbbint[seqi][0][1]].x),
					...
					if sumartp<0.5*len(seqbbint[seqi]):
						...
						if DEBUG:
							print('keep seqi',seqi,'(%d,%d-%d,%d)'%(seqbbint[seqi][-1][0]-seqbbint[seqi][0][0],seqbbint[seqi][0][0],seqbbint[seqi][-1][0],self.bbr[seqbbint[seqi][0][0]][seqbbint[seqi][0][1]].x),
							  'with',len(seqbbint[seqi]),'points','ART poss avg',sumartp/len(seqbbint[seqi]))
				#for longer seqs, stricter rule if not belong to prior defined number of tracklets pn
				elif pn>0 and seqi not in np.array([len(seqbbint[i]) for i in range(len(seqbbint))]).argsort()[-pn:][::-1]:
					sumartp = 0
					for si in range(len(seqbbint[seqi])):
				...
				if self.usebranch and pn >0:
					...
					if seqi not in seqmax:
						...
						if USE3DDIST:
							cbranchbb0s = [seqbbint[seqi][0][0]]+bbint[seqbbint[seqi][0][0]][seqbbint[seqi][0][1]].getbb()[:2]
							cbranchbb0e = [seqbbint[seqi][-1][0]]+bbint[seqbbint[seqi][-1][0]][seqbbint[seqi][-1][1]].getbb()[:2]

							mindis=99999
							for slicei in range(len(seqbbint[seqmax[0]])):
								maxbranchbb = [seqbbint[seqmax[0]][slicei][0]]+bbint[seqbbint[seqmax[0]][slicei][0]][seqbbint[seqmax[0]][slicei][1]].getbb()[:2]
								cdists = np.sqrt(np.sum([(cbranchbb0s[i]-maxbranchbb[i])**2 for i in range(len(maxbranchbb))]))
								cdiste = np.sqrt(np.sum([(cbranchbb0e[i]-maxbranchbb[i])**2 for i in range(len(maxbranchbb))]))
								if cdists<mindis:
									mindis = cdists
								if cdiste<mindis:
									mindis = cdiste
							for slicei in range(len(seqbbint[seqmax[1]])):
								maxbranchbb = [seqbbint[seqmax[1]][slicei][0]]+bbint[seqbbint[seqmax[1]][slicei][0]][seqbbint[seqmax[1]][slicei][1]].getbb()[:2]
								cdists = np.sqrt(np.sum([(cbranchbb0s[i]-maxbranchbb[i])**2 for i in range(len(maxbranchbb))]))
								cdiste = np.sqrt(np.sum([(cbranchbb0e[i]-maxbranchbb[i])**2 for i in range(len(maxbranchbb))]))
								if cdists<mindis:
									mindis = cdists
	...
	def smoothbb(self,fig=1):
		...
		for fi in range(len(dim)+self.classsize):
			for seqi in range(len(seqbbsim)):
				...
				if fi < 4:
					yseq = [getattr(self.bbs[seqbbsim[seqi][slicei][0]][seqbbsim[seqi][slicei][1]],dim[fi]) for slicei in range(len(sliceq))]
				else:
					yseq = [self.bbs[seqbbsim[seqi][slicei][0]][seqbbsim[seqi][slicei][1]].classes[fi-4] for slicei in range(len(sliceq))]
		...
		for seqi in range(len(seqbbsim)):
			for seqslicei in range(len(seqbbsim[seqi])-1,-1,-1):
				...
				if DEBUG:
					...
					probimgpatch = croppatch(self.imgstack[slicei],cbblist[1],cbblist[0],halfpatchsize,halfpatchsize)
					...
					segimgpatch = croppatch(self.gwsegstack[slicei],cbblist[1],cbblist[0],halfpatchsize,halfpatchsize)
