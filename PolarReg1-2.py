        ...
        for ei in exams:
            ...
            for slicei in caseloader.slices:
                ...
                if bi == config['batchsize']:
                    bi = 0
                    if aug == True:
                        for offi in random.sample(range(config['patchheight']), config['rottimes']):
...
def get_val_dice(cnn,config):
        ...
        for ei in val_exams:
            ...
            for slicei in caseloader.slices:
                ...
                if bi % 20 == 0:
                    ...
                    cartabel = caseloader.loadstack(slicei, 'cart_label', nei=config['depth'] // 2)[..., config['depth'] // 2]
    ...
    polarbd_all = cnn.predict(xarray_val,batch_size=config['G']*config['batchsize'])
    for ti in range(xarray_val.shape[0]):
        ...
        contourin, contourout = toctbd(polarbd, config['patchheight'], config['patchheight'])
...
logdir = DATADESKTOPdir+'/Ftensorflow/LiChen/logs/' + taskname
logdirprof = DATADESKTOPdir+'/Ftensorflow/LiChen/logs/' + taskname+'/training/plugins/profile/'
...
trainsteps = int(round(dbloader.exams*15*cfg['rottimes']//cfg['batchsize']*0.8))//800
valsteps = int(round(dbloader.exams*15//cfg['batchsize']*0.2))//160
...
if cfg['G']==1:
    cnn.save(taskname+'/'+'finalmodel.hdf5')
else:
    s_cnn.save_weights(taskname+'/'+'finalmodel.hdf5')
